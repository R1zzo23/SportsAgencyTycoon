...
namespace SportsAgencyTycoon
{
    public class Golf
    {
        public string RunTournament(Event e, World world)
        {
            ...
            /*
             * 
             * Code to run the actual tournament goes here!
             *  - remaining players play two more rounds each
             *  - sort winners
            ...
            foreach (Golfer p in tempList)
            {
                ...
                else winnerList.Add(p);
            }
            //scort resultsList from best to worst for standings sake
            resultsList = resultsList.OrderBy(o => o.CurrentScore).ToList();

            //remaining golfers play 2 more rounds
            foreach (Golfer g in winnerList)
            {
