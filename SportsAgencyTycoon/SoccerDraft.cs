...
namespace SportsAgencyTycoon
{
    public class SoccerDraft
    {
        ...
        public SoccerDraft(Random r, World w, League l)
        {
            rnd = r;
            world = w;
            league = l;
        ...
        {
            int position = rnd.Next(1, 13);
            if (position <= 4) return Position.F;
            else if (position <= 7) return Position.MID;
            else if (position <= 11) return Position.D;
            else return Position.GK;
        }
